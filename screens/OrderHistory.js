...
const OrderHistory= ({navigation}) => {
  ...
  useEffect(()=>{
    try{
      axios.post('http://192.168.207.193:3000/getMyOrder',({dname:username}))
      .then(res=>{
        console.log(result)
  ...
  useEffect(() => {
    ...
    const handleCancellOrder = async(item)=>{
      try{
        await axios.post('http://192.168.222.193:3000/cancellDealtest',({id:item._id, cropname:item.cropname, dname:item.dname}))
      }
      catch(err){
        console.log(err);
      }
    ...
    return(
        ...
        <View style={{margin:10,justifyContent:'center',alignItems:'center',borderWidth:1,backgroundColor:'black'}}>
            <Text style={{fontSize:20,color:'white'}}>Order Haistory</Text>
        </View>
      <FlatList
        data={data}
